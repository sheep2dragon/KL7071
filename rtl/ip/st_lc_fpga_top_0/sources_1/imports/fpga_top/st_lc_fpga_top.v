...
//FPGA version
`define						FPGA_TIME_LC				32'h20180629
...
wire 	[CH_NUM*2-1:0]				optic_tx_comm_fault;	//00:no fault 01: comm fault 10: crc fault
wire 	[CH_NUM-1:0]				optic_rx_comm_fault;	//0: no fault 1: comm fault
...
begin
    FLASH_SPI_CS <= spi_interface_o[3];//FMC_LPC_TX[3]  && spi_flash_switch;//if spi_flash_switch == 0 flash ctrl would be disable
    FLASH_SPI_IO_CLK <= spi_interface_o[2];//FMC_LPC_TX[2] && spi_flash_switch;
    FLASH_SPI_DI <= spi_interface_o[1];//FMC_LPC_TX[1] && spi_flash_switch;
...
IBUF #(
      ...
      .I(FMC_LPC_TX[3])       // Buffer input (connect directly to top-level port)
   );
...
assign TP22 = pll_125m_locked;
assign trans_start = sfp_rx_end_extend;
assign optic_out[9:0] = {4'd0, phy_txd[`ST_LC_CH_NUM-1:0]};
assign phy_rxd = optic_in[`ST_LC_CH_NUM-1:0];
